...
namespace Pdf.Api
{
    public class ConcateEndpoint :IConcateEndpoint 
    {
        //TODO -- GERER FICHIER DE MEME NOM
        public async Task<string> UploadFilesForConcateDocs(List<FileInfo> filesInfo, IProgress<UploadBytesProgress> progessReporter)
        {
            string res = null;
            ...
            using (var client = new HttpClient())
            {
                using (var multiForm = new MultipartFormDataContent())
                {
                    ...
                    {
                        UploadBytesProgress args = new UploadBytesProgress("http://10.0.2.2:44560/PostFilesForConcateDocs/", (int)sent, (int)total);
